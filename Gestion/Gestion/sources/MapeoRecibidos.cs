...
namespace Gestion.BusinessLogicLayer
{
	...
	public class MapeoRecibidos
	{
		...
		public static DataSet GetMapeoRecibidosRowId(string rowId, string uid, string ip, string sessionId, string expedient)
		{
            ...
            ControlLog.GetInstance().Create(OracleHelper.ExecuteNonQuery
			, new LogSystem(Convert.ToInt32(uid), "Pantalla Vista de Recibidos / Edición Correo Recibido", enuAction.Retrieve.GetDescription(), "sof_mapeo_recibidos mr, sof_empleados usr, sof_empleados addr", sSql, ip, sessionId, expedient,metadata.result));
		...
		public static int Create(int empId, int destId, string uid, string ip, string sessionId, string expedient)
		{
            ...
            ControlLog.GetInstance().Create(OracleHelper.ExecuteNonQuery
			, new LogSystem(Convert.ToInt32(uid), "Pantalla Vista de Recibidos / Edición Correo Recibido", enuAction.Create.GetDescription(), "sp_mapeo_recibidos_create", string.Join(",", oParam.Select(x => string.Format("{0}={1}", x.ParameterName, x.Value)).ToList()), ip, sessionId, expedient,metadata.result));
		...
		public static void Update(string rowId, int empId, int destId, string uid, string ip, string sessionId, string expedient)
		{
            ...
            ControlLog.GetInstance().Create(OracleHelper.ExecuteNonQuery
				, new LogSystem(Convert.ToInt32(uid), "Pantalla Vista de Recibidos / Edición Correo Recibido", enuAction.Update.GetDescription(), "sp_mapeo_recibidos_update", string.Join(",", oParam.Select(x => string.Format("{0}={1}", x.ParameterName, x.Value)).ToList()), ip, sessionId, expedient, metadata.result));
        }
		...
		public static void Remove(string rowId, string uid, string ip, string sessionId, string expedient)
		{
            ...
            ControlLog.GetInstance().Create(OracleHelper.ExecuteNonQuery
			, new LogSystem(Convert.ToInt32(uid), "Pantalla Vista de Recibidos / Edición Correo Recibido", enuAction.Delete.GetDescription(), "sp_mapeo_recibidos_remove", string.Join(",", oParam.Select(x => string.Format("{0}={1}", x.ParameterName, x.Value)).ToList()), ip, sessionId, expedient, metadata.result));
            OracleHelper.ExecuteNonQuery(ConfigurationManager.AppSettings["ConnectionString"],
