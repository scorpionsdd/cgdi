...
{
	/// <summary>
	/// Summary description for edit_mapeo_recibidos.
	/// </summary>
	public class edit_mapeo_recibidos : System.Web.UI.Page
	{
        ...
        protected System.Web.UI.HtmlControls.HtmlSelect cboArea_;
        ...
        private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack) 
			{
                ControlLog.GetInstance().Create(OracleHelper.ExecuteNonQuery, new LogSystem(Convert.ToInt32(Session["uid"]), "Pantalla Vista de Recibidos / Edición Correo Recibido", enuAction.Navegation.GetDescription(), string.Empty, string.Empty, Request.UserHostAddress, Session["sessionId"].ToString(), Session["employeeId"].ToString()));
                SetDefaultValues();
			} 
			else 
			{
		...
		{
            string lid = Request.RawUrl.Substring(Request.RawUrl.IndexOf("=")).Replace("=","");
			DataSet ds = MapeoRecibidos.GetMapeoRecibidosRowId(lid, Convert.ToString(Session["uid"]), Request.UserHostAddress, Session["sessionId"].ToString(), Session["employeeId"].ToString());
			if (ds.Tables[0].Rows.Count > 0 ) 
			{
				txtEmpleadoId.Value	= ds.Tables[0].Rows[0]["id_empleado"].ToString();
				txtName.Value		= ds.Tables[0].Rows[0]["nombre"].ToString();
				txtAddrId.Value  = ds.Tables[0].Rows[0]["id_destinatario"].ToString();
        ...
        private void LoadAreas_()
        {
            cboArea_.DataSource = Areas.GetArea_();
            cboArea_.DataValueField = "Cve_area";
            cboArea_.DataTextField = "area";
            cboArea_.DataBind();
            cboArea_.Items.Insert(0, new ListItem("» Seleccione", ""));
        }
		...
		{
			int nAddrId = int.Parse(cboAddressee.Value);
			int nEmployeeId = 0;

			if (Request.QueryString["id"].ToString().Length == 0 ) {
				nEmployeeId = int.Parse(cboEmployee.Value);
				MapeoRecibidos.Create(nEmployeeId, nAddrId, Convert.ToString(Session["uid"]), Request.UserHostAddress, Session["sessionId"].ToString(), Session["employeeId"].ToString());
			} else {

                string lid = Request.RawUrl.Substring(Request.RawUrl.IndexOf("=")).Replace("=", "");
                MapeoRecibidos.Update(lid, nEmployeeId, nAddrId, Convert.ToString(Session["uid"]), Request.UserHostAddress, Session["sessionId"].ToString(), Session["employeeId"].ToString());
			}
		...
		{
			MapeoRecibidos.Remove(Request.QueryString["id"].ToString(), Convert.ToString(Session["uid"]), Request.UserHostAddress, Session["sessionId"].ToString(), Session["employeeId"].ToString());
		}
