...
{
	/// <summary>
	/// Summary description for view_regla.
	/// </summary>
	public class view_regla : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (Session["rol"] != null)
			{
				if (Session["rol"].ToString() == "Administrator") 
				{
					if (!IsPostBack) 
					{
						ControlLog.GetInstance().Create(OracleHelper.ExecuteNonQuery, new LogSystem(Convert.ToInt32(Session["uid"]), "Pantalla Vista de Reglas/ Lista de Reglas", enuAction.Navegation.GetDescription(), string.Empty, string.Empty, Request.UserHostAddress, Session["sessionId"].ToString(), Session["employeeId"].ToString()));
						SetDefaultValues();
					} 
					else 
					{
		...
		{
			grdItems.DataSource = Regla.GetReglas(txtCutDate.Value, txtFilter.Value, Convert.ToString(Session["uid"]), Request.UserHostAddress);
			grdItems.DataBind();
		}
