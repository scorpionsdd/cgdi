...
{
	/// <summary>
	/// Summary description for view_mapeo_enviados.
	/// </summary>
	public class view_mapeo_enviados : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (Session["rol"] != null) 
			{
				if (Session["rol"].ToString() == "Administrator") 
				{
					if (!IsPostBack) 
					{
                        ControlLog.GetInstance().Create(OracleHelper.ExecuteNonQuery, new LogSystem(Convert.ToInt32(Session["uid"]), "Pantalla Vista de Enviados / Lista Enviados", enuAction.Navegation.GetDescription(), string.Empty, string.Empty, Request.UserHostAddress, Session["sessionId"].ToString(), Session["employeeId"].ToString()));
                        SetDefaultValues();
					} 
					else 
					{
		...
		{
			grdItems.DataSource = MapeoRegla.GetMapeoRegla(txtFilter.Value, Convert.ToString(Session["uid"]), Request.UserHostAddress, Session["sessionId"].ToString(), Session["employeeId"].ToString());
			grdItems.DataBind();
		}
